...
const PostContents = ({ post, isProfilePage, creatorProfile }) => {
    ...
    const authUser = userSlice((state) => state.user); // 인증된 사용자 가져오기
            ...
            {!isProfilePage && (
                <>
                    ...
                    {post.comments.length > 0 && (
                        <div className="text-sm text-gray-500 cursor-pointer" onClick={() => setIsModalOpen(true)}>
                            View all {post.comments.length} comments
                        </div>
                    )}
                    {/* 댓글 모달 */}
                    {isModalOpen && <CommentsModal isOpen={isModalOpen} onClose={() => setIsModalOpen(false)} post={post} />}
                </>
            )}
